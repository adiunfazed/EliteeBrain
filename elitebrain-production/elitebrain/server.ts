...
async function generateCoachReply(
  ...
  const systemPrompt = `You are "Elite Brain AI Coach", a friendly, articulate, highly intelligent cognitive performance specialist, neuroscientist, and supportive AI assistant.
You are conversing with a user undergoing the Elite Brain Cognitive Training Protocol.
...
5. NEVER repeat static boilerplate text or canned templates when the user asks a specific question.`;
  ...
  if (mode === 'audit') {
    prompt = `Generate my comprehensive Elite Brain Cognitive Audit. Analyze my current module levels and daily streak, identify my cognitive strong points and growth opportunities, and provide 3 tailored daily habits for maximum neuroplasticity.`;
  }

  // 1. Attempt Grok AI (xAI API) if key is configured
  if (grokKey) {
    try {
  ...
  // 2. Attempt Gemini AI if key is configured
  if (geminiKey) {
    try {
  ...
  // 3. Fallback Neuro-Coach Engine (Zero-failure guaranteed fallback)
...
function generateIntelligentFallback(userProfile: any, prompt: string, mode: string): string {
  ...
  if (mode === 'audit') {
    return `**Elite Brain Cognitive Audit — Day ${day} Protocol**\n\n• **Cognitive Strengths:** Solid consistency with a ${streak}-day training streak. Your executive control and working memory retention show steady neuroplastic adaptation across active modules.\n• **High-Yield Growth Area:** Focus on minimizing cognitive interference during high-speed Stroop and N-Back transitions to expand processing bandwidth under pressure.\n• **Recommended Daily Habit:** Complete a 2-minute Stillness Box-Breathing session immediately after training to accelerate parasympathetic recovery and memory consolidation.`;
  }
  ...
  if (lowerPrompt.includes('chess') || lowerPrompt.includes('elo') || lowerPrompt.includes('tactics') || lowerPrompt.includes('opening')) {
    return `**Chess & Neuroplastic Performance**:\n\n` +
  ...
  return `**AI Performance Insights for: "${prompt}"**\n\n` +
    ...
    `• **Current Progress**: Your ${streak}-day training streak is laying down strong neuroplastic pathways. Keep pushing for high trial accuracy!`;
}
...
async function startServer() {
    ...
    try {
      ...
      if (!userProfile?.isProUser) {
        return res.status(403).json({
          error: 'AI Coach is an Elite Brain Pro feature. Upgrade to Pro to unlock AI guidance.',
        });
      }
  ...
  // Pro Subscription / Payment Gateway Endpoints
  app.post('/api/subscription/create-checkout', paymentLimiter, (req, res) => {
    try {
      const { plan, gateway, currency } = req.body; // 'monthly' or 'annual'
      const isINR = currency === 'INR';
      
      let amount = 999; // $9.99
      if (plan === 'annual') {
        amount = isINR ? 249900 : 7999; // ₹2,499.00 or $79.99
      } else {
        amount = isINR ? 29900 : 999; // ₹299.00 or $9.99
      }

      // Generate a mock or real order ID suitable for Razorpay / Stripe / PayPal
      const orderId = `order_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;

      res.json({
        success: true,
        orderId,
        sessionUrl: `/pro-activate?status=success&plan=${plan}`,
        checkoutDetails: {
          amount,
          currency: isINR ? 'INR' : 'USD',
          plan,
          gateway: gateway || 'UPI / Razorpay / PayPal',
  ...
  // UPI QR & Intent Code Generator endpoint for Instant Paytm/PhonePe Scanning
  app.post('/api/subscription/upi-qr', paymentLimiter, (req, res) => {
    try {
      const { plan } = req.body;
      const amountINR = plan === 'annual' ? '2499' : '299';
      const payeeName = 'Elite Brain AI Pro';

      // The payee is server-controlled ONLY. Never accept it from the request
      // body — a caller could otherwise mint branded QR codes paying themselves.
      const defaultVpa = process.env.MERCHANT_UPI_ID;
      if (!defaultVpa) {
        console.error('MERCHANT_UPI_ID is not set — refusing to generate a payment QR.');
        return res.status(503).json({ error: 'Payments are not configured.' });
      }

      // Standard UPI Intent String according to NPCI specs
      const upiIntentString = `upi://pay?pa=${encodeURIComponent(
        defaultVpa
      )}&pn=${encodeURIComponent(payeeName)}&am=${amountINR}&cu=INR&tn=EliteBrain%20Pro%20Subscription`;

      // Quick-Chart QR Code Image API (returns valid scannable QR PNG)
      const qrImageUrl = `https://quickchart.io/qr?text=${encodeURIComponent(
        upiIntentString
      )}&size=250&caption=Scan+with+Paytm+/+PhonePe+/+GooglePay`;
  ...
  // Verify payment. This previously returned verified:true for ANY input,
  // including an empty body — anyone could unlock Pro for free.
  //
  // It now performs the real Razorpay HMAC-SHA256 check. If no secret is
  // configured it fails CLOSED rather than granting access. UPI transaction IDs
  // cannot be verified from the client at all and are never trusted here; a real
  // UPI flow must confirm via a gateway webhook server-side.
  app.post('/api/subscription/verify-payment', paymentLimiter, (req, res) => {
    try {
      const { paymentId, orderId, signature } = req.body;
      const secret = process.env.RAZORPAY_KEY_SECRET;

      if (!secret) {
        console.error('RAZORPAY_KEY_SECRET is not set — denying payment verification.');
      ...
      if (!paymentId || !orderId || !signature) {
        return res.status(400).json({ success: false, verified: false, error: 'Missing payment details.' });
      }
      ...
      if (!verified) {
        console.warn('Rejected payment with an invalid signature. order=%s', orderId);
        return res.status(400).json({ success: false, verified: false, error: 'Payment verification failed.' });
      }
