...
export const AICoachSection: React.FC<AICoachSectionProps> = ({
  ...
  useEffect(() => {
    if (messages.length === 0) {
      const welcomeMessage: CoachChatMessage = {
        id: 'welcome_1',
        sender: 'coach',
        text: `Hello Athlete! 👋\n\nI am your **AI Neuroplasticity & Performance Coach**. I analyze your live training metrics (Composite Index: ${brainScore}, Day ${profile.currentDay} of 30) to optimize your working memory, reaction speed, and daily focus protocols.\n\nHow can I help you elevate your performance today?`,
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      const replyText =
        data.reply ||
        "Based on your latest session metrics, your working memory accuracy is holding strong. Try increasing session tempo to push neuroplastic adaptation.";
      ...
      const fallbackMsg: CoachChatMessage = {
        id: `coach_${Date.now()}`,
        sender: 'coach',
        text: "My neural network is recalibrating. Remember to maintain high trial accuracy before accelerating response latency!",
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      };
  ...
  const suggestionChips = [
    ...
    'Explain neuroplasticity routines',
  ];
          ...
          {isTyping && (
            <div className="flex items-start gap-3">
              ...
              <div className="p-3 bg-[#171B22] border border-[#2A313C] text-[#98A2B3] rounded-2xl text-xs font-mono flex items-center gap-2">
                <span className="animate-pulse">Analyzing neural metrics...</span>
              </div>
            </div>
          )}
        ...
        {!profile.isProUser && (
          <div className="absolute inset-x-0 bottom-0 top-36 bg-[#0D1117]/85 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center z-20 space-y-3">
            ...
            <h3 className="text-lg font-display font-bold text-white">
              Unlock AI Neuroplasticity Coach
            </h3>
            <p className="text-xs text-[#98A2B3] max-w-md">
              Upgrade to Elite Brain Pro to get personalized trial analyses, custom training schedules, and real-time AI guidance.
            </p>
            <button
              onClick={() => {
                soundFx.playClick();
