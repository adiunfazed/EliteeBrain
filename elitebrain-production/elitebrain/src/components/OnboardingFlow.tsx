...
export const OnboardingFlow: React.FC<OnboardingFlowProps> = ({
  ...
  const [selectedFocusGoal, setSelectedFocusGoal] = useState<string>('Memory & Focus');
              ...
              >
                <div className="text-center max-w-xl mx-auto space-y-2">
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-signal/15 border border-signal/30 text-signal text-xs font-mono font-bold rounded-full uppercase tracking-wider mb-2">
                    ...
                    <span>Neuroplasticity Gym for Peak Mental Performance</span>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                  ...
                  <div className="p-3 bg-surface-sunk border border-rule rounded-xl flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-[11px] text-ink-muted">Track your composite Brain Index score and unlock achievement badges.</span>
                    </div>
                  </div>
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="bg-surface-sunk border border-rule rounded-2xl p-4 sm:p-5 space-y-4">
                  ...
                  {(() => {
                    ...
                    let description =
                      'Your performance matches the target challenge window. Consolidating neural memory span.';
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-surface-sunk border border-rule rounded-2xl space-y-2">
                    ...
                    <div className="space-y-1.5">
                      {[
                        'Memory & Focus',
                        'Processing Speed & Reflexes',
                        'Fluid Logic & Problem Solving',
                        'Executive Control & Flexibility',
                      ].map((goal) => (
                        <button
                          key={goal}
                          onClick={() => {
                            soundFx.playClick();
                            setSelectedFocusGoal(goal);
                          }}
                          className={`w-full text-left p-2.5 rounded-xl text-xs font-mono font-medium transition-all cursor-pointer flex items-center justify-between ${
                            selectedFocusGoal === goal
                              ? 'bg-signal/20 text-signal border border-signal/40 font-bold'
                              : 'bg-surface hover:bg-surface-sunk border border-rule text-ink-muted'
                          }`}
                        >
                          <span>{goal}</span>
                          {selectedFocusGoal === goal && <Check className="w-3.5 h-3.5 text-signal" />}
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-surface-sunk border border-rule rounded-2xl space-y-3">
                    ...
                    <div className="p-3 bg-surface border border-rule rounded-xl text-xs font-mono text-ink-muted space-y-1">
                      ...
                      <p className="text-[11px]">
                        Targeting <strong>{selectedFocusGoal}</strong> with <strong>{selectedMinutes} mins/day</strong>.
                      </p>
                    </div>
                  </div>
                </div>
