...
export const AICoachModal: React.FC<Props> = ({
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      const welcomeMessage: CoachChatMessage = {
        id: 'welcome_1',
        sender: 'coach',
        text: `Hello Athlete! 👋\n\nI am your **AI Cognitive & Neuroplasticity Coach**. I monitor your brain training metrics (Composite Index: ${brainScore}, Day ${profile.currentDay} of 30) and provide targeted advice to boost memory, focus, and mental speed.\n\nHow can I help optimize your performance today?`,
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      const replyText =
        data.reply ||
        `Regarding "${query}": Keep training consistently! High accuracy on Day ${profile.currentDay} accelerates neuroplastic adaptation.`;
          ...
          {!profile.isProUser && (
            <div className="p-4 bg-gradient-to-r from-amber-500/10 via-amber-600/10 to-cyan-500/10 border border-amber-500/40 rounded-2xl text-center space-y-2 mt-4 shadow-md">
              ...
              <p className="text-xs text-slate-300 leading-relaxed max-w-sm mx-auto">
                Upgrade to Pro Suite to get unlimited custom AI coaching, deep neuroplasticity audits, and personalized training plans.
              </p>
              <button
                onClick={() => {
                  soundFx.playClick();
