    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        if (isAdminUser) {
          ...
          // Every new account gets one month of full Pro access. The start
          // timestamp is written once and never reset, so signing out and back
          // in cannot restart the clock.
          Object.assign(newProf, startTrialFields());
          newProf.isProUser = true;
          setProfile(newProf);
          saveProfile(newProf);
