...
import { Settings, RotateCcw, Image as ImageIcon, Check } from 'lucide-react';
...
export const SlidingPuzzle: React.FC<{ profile: UserProfile, onProfileUpdate?: (p: UserProfile) => void }> = ({ profile, onProfileUpdate }) => {
  ...
  const checkWin = () => {
    setTiles((currentTiles) => {
      const won = currentTiles.every((t) => t.currentPos === t.correctPos);
      if (won && !isWon) {
        setIsWon(true);
        setIsPlaying(false);
        soundFx.playSuccess();
        if (onProfileUpdate) {
          const xpGained = gridSize === 3 ? 15 : gridSize === 4 ? 25 : 40;
          ...
          <AnimatePresence>
            {tiles.map((tile) => {
              ...
              return (
                ...
                >
                  <div className="w-full h-full rounded-lg overflow-hidden shadow-sm relative group">
                    <div 
