...
export const Dashboard: React.FC<Props> = ({
  ...
  const [hubPane, setHubPane] = useState<'tasks' | 'focus' | 'community'>('tasks');
        ...
        {activeSection === 'hub' && (
          ...
          >
            ...
            <div className="flex items-center gap-1.5">
              {(['tasks', 'focus', 'community'] as const).map((p) => (
                <button
                  key={p}
                  onClick={() => setHubPane(p)}
                  className={`text-[11px] font-mono font-bold px-3.5 py-2 rounded-xl border transition-all ${
                    hubPane === p
                      ? 'text-emerald-300 bg-emerald-500/15 border-emerald-500/40'
                      : 'text-[#98A2B3] bg-[#0E1116] border-[#2A313C] hover:border-[#3A424F]'
                  }`}
                >
                  {p === 'tasks' ? 'Tasks' : p === 'focus' ? 'Focus' : 'Community'}
                </button>
              ))}
            </div>

            {hubPane === 'tasks' && <TasksSection userId={currentUser?.uid || null} />}
            {hubPane === 'focus' && <FocusSection userId={currentUser?.uid || null} />}
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative py-1.5 px-0.5 rounded-lg flex flex-col items-center justify-center gap-0.5 transition-all cursor-pointer min-h-[44px] ${
                  isActive
