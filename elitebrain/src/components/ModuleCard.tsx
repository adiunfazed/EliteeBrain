...
export const ModuleCard: React.FC<Props> = ({ config, state, isProUser, index = 0, onLaunch }) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative flex flex-col justify-between bg-surface border rounded-2xl p-4 sm:p-5 transition-all duration-150 cursor-pointer select-none touch-manipulation overflow-hidden ${
        isPressed
    ...
    >
      ...
      <div className="pl-2 pt-2.5 border-t border-rule flex items-center justify-between gap-2 relative z-10">
        ...
        <button
          ...
          className={`px-3.5 py-1.5 font-mono text-xs font-bold rounded-xl flex items-center gap-1.5 transition-all cursor-pointer shadow-xs ${
            isLocked
