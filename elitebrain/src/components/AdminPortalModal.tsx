...
export const AdminPortalModal: React.FC<Props> = ({
    ...
    try {
      ...
      userSnap.forEach((d) => {
        ...
        userList.push({
          ...
          photoURL: u.photoURL,
          isProUser: u.profile?.isProUser || false,
    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        const updatedProfile = {
          ...
          proExpiryDate: newProState ? Date.now() + 365 * 24 * 60 * 60 * 1000 : null,
        };
        await updateDoc(userDocRef, { profile: updatedProfile });

        soundFx.playSuccess();
        setActionSuccess(`Pro status ${newProState ? 'GRANTED' : 'REVOKED'} for ${userRec.email}`);
        setTimeout(() => setActionSuccess(null), 3000);
        await loadData();
        onRefreshProfile();
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-6 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-4xl bg-[#171B22] border border-[#2A313C] rounded-3xl p-5 md:p-7 shadow-2xl text-left overflow-hidden max-h-[90vh] flex flex-col text-[#F4F6F8]">
        {/* Top Header */}
        <div className="flex items-center justify-between pb-4 border-b border-[#2A313C] shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-xl bg-[#5C6CF2]/20 border border-[#5C6CF2]/40 flex items-center justify-center text-[#5C6CF2]">
              <Shield className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-lg font-black text-[#F4F6F8] flex items-center gap-2">
                <span>Cloud Admin & User Management Portal</span>
                <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-[#5C6CF2]/20 text-[#5C6CF2] border border-[#5C6CF2]/40">
                  ADMIN ONLY
                </span>
              </h2>
        ...
        <div className="flex gap-2 mt-4 shrink-0">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              activeTab === 'PAYMENTS'
                ? 'bg-[#5C6CF2] text-white shadow-md'
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3 shrink-0">
            ...
            {isEditingUpi && (
              <form onSubmit={handleSaveUpi} className="mt-3 pt-3 border-t border-[#2A313C] flex gap-2">
                <input
                  ...
                  className="flex-1 bg-[#171B22] border border-[#2A313C] rounded-xl px-3 py-1.5 text-xs font-mono text-[#F4F6F8] focus:border-[#5C6CF2] focus:outline-none"
                />
                <button
                  type="submit"
                  disabled={savingUpi}
            ...
            {isEditingTelegram && (
              <form onSubmit={handleSaveTelegramLink} className="mt-3 pt-3 border-t border-[#2A313C] flex gap-2">
                <input
                  ...
                  className="flex-1 bg-[#171B22] border border-[#2A313C] rounded-xl px-3 py-1.5 text-xs font-mono text-[#F4F6F8] focus:border-sky-400 focus:outline-none"
                />
                <button
                  type="submit"
            ...
            {isEditingYoutube && (
              <form onSubmit={handleSaveYoutubeLink} className="mt-3 pt-3 border-t border-[#2A313C] flex gap-2">
                <input
                  ...
                  className="flex-1 bg-[#171B22] border border-[#2A313C] rounded-xl px-3 py-1.5 text-xs font-mono text-[#F4F6F8] focus:border-red-400 focus:outline-none"
                />
                <button
                  type="submit"
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 shrink-0">
          {activeTab === 'PAYMENTS' ? (
            ...
            <div className="text-xs font-bold text-[#5C6CF2] font-mono flex items-center gap-2">
              <Mail className="w-4 h-4 text-[#5C6CF2]" />
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1 sm:w-64">
              ...
              <input
                ...
                className="w-full bg-[#0E1116] border border-[#2A313C] rounded-xl pl-9 pr-3 py-1.5 text-xs text-[#F4F6F8] placeholder:text-[#98A2B3] focus:border-[#5C6CF2] focus:outline-none"
              />
            </div>
            ...
            <button
              onClick={loadData}
              title="Refresh data"
              className="p-2 rounded-xl bg-[#0E1116] border border-[#2A313C] hover:bg-[#171B22] text-[#98A2B3] hover:text-[#F4F6F8] transition-all cursor-pointer"
            >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-xs text-[#98A2B3] font-mono">
                        <span>
                          Plan: <strong className="text-[#5C6CF2]">{req.plan.toUpperCase()}</strong> (₹{req.amountINR})
                        </span>
                        <span>•</span>
            ...
            filteredUsers.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {filteredUsers.map((u) => (
                  <div
                    key={u.uid}
                    className="p-3.5 rounded-2xl bg-[#0E1116] border border-[#2A313C] hover:border-[#5C6CF2]/40 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3"
                  >
                    <div className="flex items-center gap-3">
                      {u.photoURL ? (
                        <img src={u.photoURL} alt={u.displayName} className="w-9 h-9 rounded-full border border-[#5C6CF2]/40 object-cover shrink-0" />
                      ) : (
                        <div className="w-9 h-9 rounded-full bg-[#5C6CF2]/20 border border-[#5C6CF2]/40 text-[#5C6CF2] font-bold text-xs flex items-center justify-center shrink-0">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-black text-[#F4F6F8]">{u.displayName}</span>
                          <span className="text-[9px] font-mono px-2 py-0.5 rounded-full bg-[#171B22] border border-[#2A313C] text-[#98A2B3] uppercase">
                            {u.authProvider}
                          </span>
                          {u.isProUser && (
                            <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40 flex items-center gap-1">
                              <Crown className="w-3 h-3 text-amber-400" />
                              PRO
                            </span>
                          )}
                        </div>
                        <div className="text-xs font-mono text-[#5C6CF2] mt-0.5">{u.email}</div>
                    ...
                    <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                      ...
                      >
                        ...
                        <span>{u.isProUser ? 'Revoke Pro' : 'Grant 1-Yr Pro'}</span>
                      </button>
                    </div>
                  </div>
                ))}
