...
import { EliteBrainLogo } from './EliteBrainLogo';
    ...
    <header className="sticky top-0 z-40 bg-[#0E1116]/95 backdrop-blur-xl border-b border-[#2A313C] px-2.5 sm:px-4 md:px-6 py-2 transition-all select-none shadow-md overflow-x-clip">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-1.5 sm:gap-3">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0 min-w-0">
          <EliteBrainLogo size="sm" showSubtext={false} />
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <h1 className="text-xs xs:text-sm sm:text-base md:text-lg lg:text-xl font-mono font-black tracking-wider flex items-center gap-0.5 sm:gap-1 whitespace-nowrap">
              ...
              <span className="text-[#5C6CF2]">BRAIN</span>
            </h1>
            ...
            <div className="hidden md:block">
              {profile.isProUser ? (
                <button
                  ...
                  className="text-[10px] font-mono font-bold text-[#818CF8] px-2 py-0.5 bg-[#1E1B4B] hover:bg-[#2A2468] border border-[#5C6CF2]/40 rounded-full tracking-wide flex items-center gap-1 shrink-0 cursor-pointer transition-all active:scale-95"
                >
        ...
        <div className="hidden md:flex items-center gap-2.5">
          ...
          >
            <HelpCircle className="w-3.5 h-3.5 text-[#5C6CF2]" />
          ...
          <button
            ...
            className={`p-2 border rounded-xl transition-all cursor-pointer active:scale-95 ${
              isColorblind
                ? 'bg-[#5C6CF2] text-white border-[#5C6CF2] shadow-sm'
          ...
          {currentUser ? (
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-[#5C6CF2] hover:bg-[#4B5BE0] text-white font-mono text-xs font-bold px-3.5 py-1.5 rounded-xl cursor-pointer transition-all active:scale-95 shadow-sm"
            >
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 md:hidden shrink-0">
          ...
          {profile.isProUser ? (
            <motion.button
              ...
              className="text-[10px] font-mono font-bold text-[#818CF8] px-1.5 sm:px-2 py-0.5 sm:py-1 bg-[#1E1B4B] hover:bg-[#2A2468] border border-[#5C6CF2]/40 rounded-lg sm:rounded-xl tracking-wide flex items-center gap-0.5 sm:gap-1 shrink-0 cursor-pointer transition-all"
            >
              <Crown className="w-3 h-3 text-amber-400" />
          ...
          <button
            ...
            className={`p-1 sm:p-1.5 border rounded-lg sm:rounded-xl transition-all cursor-pointer active:scale-95 flex items-center justify-center shrink-0 ${
              isMobileMenuOpen
                ? 'bg-[#5C6CF2] text-white border-[#5C6CF2]'
                : 'bg-[#171B22] border-[#2A313C] text-[#F4F6F8]'
            }`}
          >
            {isMobileMenuOpen ? <X className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> : <Sliders className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#818CF8]" />}
          </button>
        </div>
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              >
                <HelpCircle className="w-4 h-4 text-[#5C6CF2]" />
                <span>Methods</span>
              </button>
            ...
            <div className="mt-3 pt-3 border-t border-[#2A313C]/60 space-y-2.5">
              ...
              <div className="bg-[#171B22] border border-[#2A313C] p-2.5 rounded-2xl space-y-2">
                <div className="flex items-center justify-between text-xs font-mono text-[#98A2B3]">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <Edit2 className="w-2.5 h-2.5 text-[#818CF8]" />
                ...
                {isEditingName ? (
                  <form onSubmit={handleSaveName} className="flex items-center gap-2 pt-1">
                    <input
                      ...
                      maxLength={30}
                      className="flex-1 bg-[#0E1116] border border-[#2A313C] rounded-xl px-3 py-1.5 text-xs font-mono text-slate-100 focus:outline-none focus:border-[#5C6CF2]"
                    />
                    <button
                      type="submit"
                      disabled={isSavingName || !nameInput.trim()}
                      className="px-3 py-1.5 bg-[#5C6CF2] hover:bg-[#4B5BE0] text-white font-mono text-xs font-bold rounded-xl cursor-pointer transition-all disabled:opacity-50 shrink-0 flex items-center gap-1"
                    >
                  ...
                  <p className="text-xs font-mono text-slate-200 font-bold pl-1 truncate">
                    {profile.displayName || currentUser?.displayName || currentUser?.email?.split('@')[0] || 'Elite Athlete'}
                  </p>
                )}
              </div>
              ...
              {currentUser ? (
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-[#5C6CF2] text-white font-mono text-xs font-bold px-3.5 py-2 rounded-xl shadow-sm active:bg-[#4B5BE0] cursor-pointer"
                >
