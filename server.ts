 ...
 *
 * Returns null when no key is configured, so callers fail cleanly rather than
 * constructing a client that cannot work.
 */
function makeGeminiClient(): any | null {
...
async function startServer() {
   ...
   *
   * Pro-gated like the chat coach, since it uses the same paid quota. Images
   * are passed straight to the model and never written to disk — these are
   * photographs of people's bodies and homes.
   */
  app.post('/api/coach/vision', coachLimiter, async (req, res) => {
    if (!isAdminAvailable()) {
      return res.status(503).json({ error: 'Unavailable right now.' });
    }
    ...
    try {
        ...
        return res.status(403).json({
          error:
            verified.status === 'expired'
              ? 'Your free month has ended. Unlock Pro to keep using Coach tools.'
              : 'Coach tools are a Pro feature. Start your free month to unlock them.',
          status: verified.status,
        });
      }

      const { tool, image, mimeType } = req.body || {};

      if (!isAllowedMime(String(mimeType))) {
      ...
      // Base64 inflates by roughly a third, so check the decoded size.
