...
export const GoalsSection: React.FC<Props> = ({ userId, tasks = [], onStartFocus }) => {
  ...
  useEffect(() => {
    ...
    for (const g of goals.filter((x) => x.status === 'active')) {
      const p = goalProgress(g, habits, logs, today, tasks);
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {editingHabitId === habit.id ? (
                  <input
                    ...
                    className="bg-[#171B22] border border-[#5C6CF2]/60 rounded-lg px-2 py-1 text-sm text-[#F4F6F8] outline-none min-w-0 flex-1"
                  />
                ) : (
                  <span
          ...
          {expandedHabit === habit.id && !compact && (
            ...
            >
              <div className="px-3.5 pb-3.5 space-y-3">
                ...
                <div className="flex items-center gap-2 flex-wrap pt-1">
                  ...
                  {habit.status === 'active' ? (
                    ...
                    <button
                      ...
                      className="eb-press text-[10px] font-mono font-bold px-2.5 py-2 rounded-lg border border-emerald-500/30 text-emerald-300 flex items-center gap-1.5"
                    >
                      Restore
                    </button>
                  )}
                ...
                <p className="text-[9px] font-mono text-[#5A6472]">
                  Archiving keeps your streak history. Deleting removes it permanently.
                </p>
              </div>
            </motion.div>
          )}
  ...
  const GoalCard: React.FC<{ goal: Goal }> = ({ goal }) => {
    const progress = goalProgress(goal, habits, logs, today, tasks);
