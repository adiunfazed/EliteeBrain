  ...
  if (isFullscreen) {
    ...
    return (
      <div className="fixed inset-0 z-[60] bg-[#07090D] flex flex-col items-center justify-between p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        ...
        <AnimatePresence>
          {gameStatus !== 'playing' && (
            ...
            >
              ...
              >
                {gameStatus === 'won' || gameStatus === 'timeout_win' ? (
                  <Trophy className="w-11 h-11 shrink-0" />
                ) : gameStatus === 'drawn' ? (
                  <ShieldAlert className="w-11 h-11 shrink-0" />
                ) : (
                  <RotateCcw className="w-11 h-11 shrink-0" />
                )}
              </div>
        ...
        {/* Opponent row */}
        <div className="w-full max-w-full max-w-[640px] flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {game.inCheck() && gameStatus === 'playing' && (
              <span className="t-meta font-black px-2 py-0.5 rounded-full bg-rose-500/20 border border-rose-500/40 eb-danger shrink-0">
                CHECK
              </span>
            )}
          </div>
        ...
        {/* Controls */}
        <div className="w-full max-w-full max-w-[640px] flex items-center gap-2 mt-2">
          <button
            onClick={() => setIsFullscreen(false)}
            className="eb-press flex-1 min-w-0 py-3 rounded-xl bg-[var(--signal)] hover:brightness-110 text-white text-xs font-mono font-black flex items-center justify-center gap-1.5 min-h-[44px]"
          >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 bg-rose-500/10 border border-rose-500/30 eb-danger t-meta rounded-md">
                    {engineLevel} ELO
                  </span>
                </div>
                <div className="flex items-center gap-1 mt-0.5 min-h-[18px]">
                  ...
                  {diff < 0 && (
                    <span className="t-meta eb-danger ml-1">+{Math.abs(diff)}</span>
                  )}
                </div>
              </div>
            </div>
                ...
                >
                  ...
                  >
                    {gameStatus === 'won' || gameStatus === 'timeout_win' ? (
                      ...
                      <RotateCcw className="w-12 h-12 shrink-0" />
                    )}
                  </div>
                  ...
                  {eloDelta !== null && (
                    <div className="flex items-center gap-2 mb-6 bg-[var(--surface)] border border-[var(--rule)] px-5 py-2.5 rounded-xl">
                      ...
                      <div className="flex items-center gap-1 text-sm font-mono font-black text-white">
                        ...
                        >
                          {eloDelta >= 0 ? (
                            <TrendingUp className="w-4 h-4 shrink-0 ml-1" />
                          ) : (
                            <TrendingDown className="w-4 h-4 shrink-0 ml-1" />)}
                          {eloDelta >= 0 ? `+${eloDelta}` : eloDelta}
                        </span>
                      </div>
                    </div>
                  ...
                  <button
                    onClick={startGame}
                    className="px-8 py-3.5 bg-white text-slate-950 font-mono text-xs font-bold uppercase rounded-xl hover:bg-slate-200 transition-all active:scale-95 cursor-pointer shadow-none flex items-center gap-2"
                  >
                    <RotateCcw className="w-4 h-4 shrink-0" />
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 bg-amber-500/10 border border-amber-500/30 eb-warn t-meta rounded-md">
                    {userElo} ELO
                  </span>
                </div>
                <div className="flex items-center gap-1 mt-0.5 min-h-[18px]">
                  ...
                  {diff > 0 && (
                    <span className="t-meta eb-done ml-1">+{diff}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 mt-3">
              {([
                ...
                <button
                  ...
                  disabled={disabled}
                  aria-label={label}
                  className="flex-1 min-w-0 min-h-[44px] rounded-xl border flex items-center justify-center transition-colors disabled:opacity-30"
