...
import { X, Send, Sparkles, Bot, User as UserIcon, RefreshCw, Crown, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-[92] flex items-center justify-center p-2 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn select-none">
      <div className="relative w-full max-w-2xl h-[90vh] max-h-[700px] bg-slate-900 border border-cyan-500/30 rounded-xl sm:rounded-xl shadow-none shadow-cyan-500/10 flex flex-col overflow-hidden font-sans my-auto">
        ...
        <div className="p-3.5 sm:p-4 border-b border-slate-800 bg-slate-950/90 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className="w-9 h-9 shrink-0 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-cyan-500/20 to-blue-600/30 border border-cyan-500/40 flex items-center justify-center text-cyan-400 shadow-md">
                <Bot className="w-5 h-5 shrink-0 sm:w-6 sm:h-6" />
              </div>
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {profile.isProUser ? (
                  <span className="t-meta text-cyan-400 px-1.5 py-0.5 bg-cyan-950 border border-cyan-500/40 rounded-full">
                    PRO
                  </span>
                ) : (
                  <span className="t-meta eb-warn px-1.5 py-0.5 bg-amber-950/60 border border-amber-500/40 rounded-full flex items-center gap-0.5">
                    <Crown className="w-3.5 h-3.5 shrink-0" /> Free Mode
                  </span>
                )}
              </div>
        ...
        <div className="flex-1 min-w-0 overflow-y-auto p-3.5 sm:p-5 space-y-4 bg-slate-900/60">
          {messages.map((m) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isCoach ? <Bot className="w-4 h-4 shrink-0" /> : <UserIcon className="w-4 h-4 shrink-0" />}
                </div>
        ...
        <div className="px-3 py-2 bg-slate-950/80 border-t border-slate-800 flex items-center gap-2 overflow-x-auto no-scrollbar shrink-0">
          <span className="t-meta text-slate-500 uppercase tracking-wider shrink-0 pl-1">
            Prompts:
          </span>
          <button
            onClick={() => handlePromptClick("How can I improve my working memory score?")}
            className="px-2.5 py-1 bg-slate-800/80 hover:bg-slate-800 border border-slate-700/80 text-cyan-300 t-meta rounded-lg shrink-0 cursor-pointer transition-colors"
          >
