          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="t-meta uppercase tracking-wider text-[var(--signal-ink)] px-2 py-0.5 bg-[color-mix(in_oklab,var(--signal)_15%,transparent)] border border-[var(--signal)]/30 rounded-md">
                  Your coach
                </span>
                {profile.isProUser ? (
                  <span className="t-meta eb-warn px-2 py-0.5 bg-amber-500/20 border border-amber-500/30 rounded-md">
                    Pro Active
                  </span>
                ) : (
                  <span className="t-meta text-slate-400 px-2 py-0.5 bg-slate-800 border border-slate-700 rounded-md">
                    Free Preview
                  </span>
                )}
              </div>
        ...
        <div className="flex-1 min-w-0 overflow-y-auto space-y-3 pr-2 scrollbar-thin">
          {messages.length <= 1 && !isTyping && (
            <div className="pt-2 pb-1">
              <p className="eb-label mb-2.5">Ask about</p>
              <div className="space-y-2">
                {[
                  'What should I do next?',
                  'What am I getting wrong this week?',
                  'Why do I keep falling behind?',
                ].map((prompt) => (
                  <button
                    key={prompt}
                    onClick={() => handleSendMessage(prompt)}
                    className="w-full text-left rounded-xl px-4 py-3 text-[14px] transition-colors"
                    style={{
          ...
          {messages.map((msg) => (
            ...
            >
              {msg.sender === 'coach' && (
                <div className="w-8 h-8 rounded-xl bg-[color-mix(in_oklab,var(--signal)_20%,transparent)] border border-[color-mix(in_oklab,var(--signal)_40%,transparent)] text-[var(--signal-ink)] flex items-center justify-center shrink-0 mt-1">
                ...
                {COACH_ACTIONS.map((action, i) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ color: hue }}
                      className="eb-press eb-tint text-left p-3 rounded-xl bg-[#14171F] border border-[#262C38] hover:border-current transition-colors min-w-0 relative"
                    >
