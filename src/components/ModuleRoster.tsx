...
export const ModuleRoster: React.FC<ModuleRosterProps> = ({
  ...
  return (
    <div className="w-full bg-surface border border-rule rounded-[2px] overflow-hidden">
      {/* Table Header */}
      <div className="hidden md:grid grid-cols-12 px-4 py-2 bg-surface-sunk border-b border-rule t-meta text-ink-muted uppercase tracking-[0.09em]">
        ...
        {MODULE_METADATA.map((meta, index) => {
          ...
          return (
            <motion.div
              ...
              className="group relative flex flex-col md:grid md:grid-cols-12 items-stretch min-h-[64px] px-4 py-3 md:py-0 bg-surface hover:bg-surface-sunk active:bg-[color-mix(in_oklab,var(--signal)_10%,transparent)] active:scale-[0.98] transition-all duration-120 cursor-pointer select-none touch-manipulation"
            >
              ...
              <div className="col-span-4 flex flex-col justify-center pl-2 pr-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="t-meta text-ink-muted px-1.5 py-0.5 bg-surface-sunk border border-rule rounded-[1px]">
                    {meta.domain}
                  </span>
                  {meta.isPro && (
                    <span className="inline-flex items-center gap-1 t-meta text-amber-700 dark:eb-warn">
              ...
              <div className="col-span-2 flex items-center justify-start md:justify-center">
                {isLocked ? (
                  <span className="t-meta text-signal hover:underline">
                    Included in Pro →
                  </span>
                ) : (
              ...
              <div className="col-span-1 flex items-center justify-end">
                <ArrowRight className="w-4 h-4 shrink-0 text-ink-muted group-hover:text-signal group-hover:translate-x-1 transition-transform duration-120" />
              </div>
            </motion.div>
          );
        })}
