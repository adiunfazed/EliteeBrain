    ...
    <div className="eb-panel p-4 sm:p-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-1.5 shrink-0">
          {streak.best > 0 && (
            <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-[var(--signal)]/12 border border-[var(--signal)]/30 text-[var(--signal-ink)]">
              Best {streak.best}
            </span>
          )}
      ...
      <div className="grid grid-cols-7 gap-1.5 mt-4">
        {streak.week.map((d, i) => (
          <div key={d.date} className="flex flex-col items-center gap-1.5 min-w-0">
            ...
            <span
              className={`t-meta ${
                d.isToday ? 'text-[var(--signal-ink)]' : 'text-[var(--ink-dim)]'
              }`}
            >
