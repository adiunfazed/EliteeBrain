      ...
      {pane === 'week' && (
        <div className="space-y-3">
          ...
          <div className="overflow-x-auto no-scrollbar -mx-1 px-1">
            <div className="grid grid-cols-7 gap-1.5 min-w-full max-w-[520px]">
              {weekDates.map((iso) => {
                ...
                return (
                  <div
                    ...
                    className={`rounded-xl border p-2 min-w-0 ${
                      isToday ? 'border-[var(--signal)]/45 bg-[var(--signal)]/[0.07]' : 'border-[var(--rule)] bg-[var(--ground)]'
                    }`}
                  >
                    <p className="t-meta uppercase text-center">
                      {d.toLocaleDateString(undefined, { weekday: 'short' })}
                    </p>
                    <p
                      className={`t-meta font-black text-center tabular-nums ${
                        isToday ? 'text-[var(--signal-ink)]' : 'text-[var(--ink)]'
                      }`}
                    >
                      {d.getDate()}
                    </p>
                    ...
                    <div className="mt-2 space-y-1">
                      {dayBlocks.length === 0 ? (
                        <p className="t-meta text-[var(--rule-strong)] text-center py-1">—</p>
                      ) : (
