...
export const ChartRecorder: React.FC<ChartRecorderProps> = ({ profile, className = '' }) => {
  ...
  return (
    <div className={`relative flex flex-col items-center justify-center p-2 bg-surface border border-rule rounded-[2px] ${className}`}>
      ...
      <div className="w-full flex items-center justify-between pb-2 border-b border-rule t-meta text-ink-muted">
        <span>COGNITIVE PROFILE · PEN PLOTTER</span>
        <span className="text-signal font-bold">1100MS TRACE</span>
      </div>

      <div className="relative w-full max-w-full max-w-[320px] aspect-square flex items-center justify-center my-2">
      ...
      {/* Legend */}
      <div className="w-full flex items-center justify-between pt-2 border-t border-rule t-meta text-ink-muted">
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-0.5 shrink-0 bg-signal inline-block" />
          <span>CURRENT TRACE</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-0.5 shrink-0 border-b border-dashed border-ink-muted inline-block" />
