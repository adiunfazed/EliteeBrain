        ...
        <div className="space-y-4 text-xs">
          ...
          <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-3">
            <div className="flex items-center justify-between">
              ...
              {currentUser && (
                <span className="t-meta eb-done bg-emerald-500/10 px-2 py-0.5 rounded-md border border-emerald-500/30">
                  Synced to Account
                </span>
              )}
            </div>
            ...
            <form onSubmit={handleSaveName} className="flex items-center gap-2">
              <input
                ...
                maxLength={30}
                className="flex-1 min-w-0 bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-slate-100 text-xs focus:outline-none focus:border-[var(--signal)]"
              />
              <button
                type="submit"
                disabled={isSavingName || !nameInput.trim()}
                className="px-3.5 py-2 bg-[var(--signal)] hover:bg-[#4B5BE0] text-white font-mono font-bold rounded-xl text-xs transition-all cursor-pointer disabled:opacity-50 shrink-0"
              >
