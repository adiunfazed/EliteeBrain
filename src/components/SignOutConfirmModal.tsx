  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 shadow-2xl text-center">
        ...
        <span className="text-[10px] uppercase font-mono font-bold tracking-widest px-3 py-0.5 rounded-full bg-[#8B5CF6]/10 text-[#8B5CF6] border border-[#8B5CF6]/30 inline-block mb-3">
          ACCOUNT MANAGEMENT
        </span>
        ...
        <div className="relative w-20 h-20 mx-auto mb-3">
          {user.photoURL ? (
            <img
              src={user.photoURL}
              alt="Profile"
              className="w-20 h-20 shrink-0 rounded-full border-2 border-[#8B5CF6] shadow-lg shadow-[#8B5CF6]/20 object-cover"
            />
          ) : (
            <div className="w-20 h-20 shrink-0 rounded-full bg-[#8B5CF6] text-white font-black text-2xl flex items-center justify-center border-2 border-[#8B5CF6]">
              {user.displayName ? user.displayName[0] : 'U'}
            </div>
          )}
          <span className="absolute bottom-0 right-0 p-1 bg-[#8B5CF6] text-white rounded-full shadow-md">
        ...
        <div className="bg-slate-950/90 border border-slate-800 rounded-2xl p-3 mb-4 space-y-2 text-left">
          <div className="flex items-center justify-between text-xs font-mono text-slate-300">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <Edit2 className="w-2.5 h-2.5 shrink-0 text-[#8B5CF6]" />
          ...
          {isEditing ? (
            <form onSubmit={handleSaveName} className="flex items-center gap-2 pt-1">
              <input
                ...
                maxLength={30}
                className="flex-1 min-w-0 bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-100 font-mono focus:outline-none focus:border-[#8B5CF6]"
              />
              <button
                type="submit"
                disabled={isSaving || !nameInput.trim()}
                className="px-3.5 py-1.5 bg-[#8B5CF6] hover:bg-[#4B5BE0] text-white font-mono font-bold rounded-xl text-xs transition-all cursor-pointer disabled:opacity-50 shrink-0"
              >
        ...
        {/* Status Box */}
        <div className="bg-slate-950/80 rounded-2xl border border-[#8B5CF6]/20 p-3.5 mb-5 text-left flex items-start gap-3">
        ...
        <div className="space-y-2.5">
          <button
            ...
            className="w-full py-3 px-4 rounded-xl bg-[#8B5CF6] hover:bg-[#4B5ACD] text-white font-black text-xs uppercase tracking-wider cursor-pointer shadow-lg shadow-[#8B5CF6]/20 transition-all active:scale-95"
          >
