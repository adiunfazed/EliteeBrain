    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-none text-center">
        ...
        <div className="relative w-20 h-20 mx-auto mb-3">
          ...
          <span className="absolute bottom-0 right-0 p-1 bg-[var(--signal)] text-white rounded-full shadow-md">
          ...
          {isEditing ? (
            <form onSubmit={handleSaveName} className="flex items-center gap-2 pt-1">
              <input
                ...
                maxLength={30}
                className="flex-1 min-w-0 bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-100 font-mono focus:outline-none focus:border-[var(--signal)]"
              />
              <button
                type="submit"
                disabled={isSaving || !nameInput.trim()}
                className="px-3.5 py-1.5 bg-[var(--signal)] hover:bg-[#4B5BE0] text-white font-mono font-bold rounded-xl text-xs transition-all cursor-pointer disabled:opacity-50 shrink-0"
              >
