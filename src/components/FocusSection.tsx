      ...
      >
        ...
        <div className="mt-7 flex items-center justify-center gap-2.5 flex-wrap">
          ...
          <motion.button
            ...
            className="eb-lift px-5 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 text-xs font-mono font-black flex items-center gap-2"
          >
    ...
    <div className="space-y-4">
      ...
      <div className="relative overflow-hidden bg-[#121722] border border-[var(--rule)] rounded-xl p-4 space-y-4">
        <div>
          ...
          {todayTasks.length > 0 ? (
            <div className="space-y-1.5 max-h-44 overflow-y-auto pr-0.5">
              {todayTasks.map((t) => (
                <button
                  ...
                  className={`eb-press w-full text-left px-3 py-2.5 rounded-xl border text-sm flex items-center gap-2.5 ${
                    selectedTaskId === t.id
                      ? 'bg-[color-mix(in_oklab,var(--signal)_15%,transparent)] border-[var(--signal)]/50 text-[var(--ink)] shadow-[0_0_18px_-6px_rgba(92,108,242,0.7)]'
                      : 'bg-[var(--surface-sunk)] border-[var(--rule)] text-[var(--ink-muted)] hover:border-[var(--rule-strong)]'
                  }`}
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                      selectedTaskId === t.id ? 'bg-[var(--signal)]' : 'bg-[var(--rule-strong)]'
                    }`}
                  />
            ...
            <input
              ...
              maxLength={100}
              className="w-full eb-card-sunk focus:border-[color-mix(in_oklab,var(--signal)_60%,transparent)] rounded-xl px-3 py-2.5 text-sm text-[var(--ink)] placeholder:text-[var(--ink-dim)] outline-none transition-colors"
            />
          )}
        </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {FOCUS_PRESETS.map((m) => (
              <button
                ...
                className={`eb-press px-4 py-2 rounded-xl border text-xs font-mono font-bold ${
                  minutes === m
        ...
        <motion.button
          ...
          className="eb-lift w-full py-3.5 rounded-xl bg-[var(--signal)] hover:bg-[#7C3AED] disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-mono font-black flex items-center justify-center gap-2"
        >
