    ...
    switch (color) {
      case 'amber':
        return {
          ...
          pill: 'bg-amber-500/20 eb-warn border-amber-500/40',
        };
      case 'rose':
        return {
          bg: 'bg-rose-500/15',
          ...
          pill: 'bg-rose-500/20 eb-danger border-rose-500/40',
        };
      case 'violet':
        return {
          bg: 'bg-violet-500/15',
      ...
      case 'emerald':
        return {
          ...
          pill: 'bg-emerald-500/20 eb-done border-emerald-500/40',
        };
      case 'sky':
        return {
          bg: 'bg-sky-500/15',
      ...
      default:
        return {
          bg: 'bg-[color-mix(in_oklab,var(--signal)_15%,transparent)]',
          border: 'border-[var(--signal)]/50',
          text: 'text-[var(--signal-ink)]',
          glow: 'rgba(92, 108, 242, 0.4)',
          pill: 'bg-[color-mix(in_oklab,var(--signal)_20%,transparent)] text-[var(--signal-ink)] border-[color-mix(in_oklab,var(--signal)_40%,transparent)]',
        };
    }
  };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden flex items-center justify-center opacity-30">
          <motion.div
            ...
            className="w-full max-w-[600px] h-[600px] rounded-full bg-conic-gradient from-amber-500/20 via-indigo-500/20 via-emerald-500/20 to-amber-500/20 blur-2xl"
          />
        </div>
        ...
        >
          ...
          {/* Banner Tagline */}
          <div className="inline-flex items-center gap-1.5 px-3.5 py-1 bg-amber-500/15 border border-amber-500/40 rounded-full eb-warn text-xs font-mono font-bold tracking-widest uppercase mb-6 shadow-xs animate-pulse">
            <Sparkles className="w-3.5 h-3.5 shrink-0 eb-warn" />
          ...
          <div className="flex items-center justify-center gap-2 mb-3">
            <span className={`px-2.5 py-0.5 border t-meta rounded-md ${style.pill}`}>
              {currentAchievement.category}
            </span>
            <span className="px-2.5 py-0.5 bg-amber-500/20 eb-warn border border-amber-500/40 t-meta rounded-md flex items-center gap-1">
          ...
          >
            ...
            {currentIndex < achievementsToDisplay.length - 1 && <ChevronRight className="w-4 h-4 shrink-0" />}
          </motion.button>
        </motion.div>
      </div>
    </AnimatePresence>
