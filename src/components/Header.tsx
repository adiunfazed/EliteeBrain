    ...
    <header className="sticky top-0 z-40 bg-[var(--ground)]/95 backdrop-blur-xl border-b border-[var(--rule)] px-2.5 sm:px-4 md:px-6 py-2 transition-all select-none shadow-md overflow-x-clip">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-1.5 sm:gap-3">
            ...
            <div className="hidden md:block">
              {isHydrated === false ? (
                ...
                <button
                  ...
                  className="t-meta text-[var(--signal-ink)] px-2 py-0.5 bg-[#2A1B4B] hover:bg-[#3B2468] border border-[color-mix(in_oklab,var(--signal)_40%,transparent)] rounded-full tracking-wide flex items-center gap-1 shrink-0 cursor-pointer transition-all active:scale-95"
                >
                  <Crown className="w-3.5 h-3.5 shrink-0 eb-warn" />
                ...
                <button
                  ...
                  className="t-meta eb-warn px-2 py-0.5 bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/40 rounded-full transition-all flex items-center gap-1 cursor-pointer shrink-0 shadow-xs active:scale-95"
                >
                  <Crown className="w-3.5 h-3.5 shrink-0 eb-warn" />
                  <span>Get Pro</span>
                </button>
        ...
        <div className="hidden md:flex items-center gap-2.5">
          ...
          {onToggleDarkMode && (
            ...
            >
              {isDarkMode ? (
                <Sun className="w-4 h-4 shrink-0 text-[var(--signal-ink)]" />
              ) : (
                <Moon className="w-4 h-4 shrink-0 text-[var(--signal-ink)]" />
              )}
            </button>
          )}
          ...
          >
            {profile.soundEnabled ? (
              <Volume2 className="w-4 h-4 shrink-0 text-[var(--signal-ink)]" />
            ) : (
          ...
          {currentUser ? (
            <button
              ...
              className="flex items-center gap-2 bg-[var(--surface-sunk)] hover:bg-[#212631] border border-[var(--rule)] px-3.5 py-1.5 rounded-xl font-mono text-xs text-[var(--ink)] cursor-pointer transition-all active:scale-95 max-w-full max-w-[140px]"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-[var(--signal)] hover:bg-[#4B5BE0] text-white font-mono text-xs font-bold px-3.5 py-1.5 rounded-xl cursor-pointer transition-all active:scale-95 shadow-sm"
            >
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 md:hidden shrink-0">
          ...
          {isHydrated === false ? (
            ...
            <motion.button
              ...
              className="t-meta text-[var(--signal-ink)] px-1.5 sm:px-2 py-0.5 sm:py-1 bg-[#2A1B4B] hover:bg-[#3B2468] border border-[color-mix(in_oklab,var(--signal)_40%,transparent)] rounded-lg sm:rounded-xl tracking-wide flex items-center gap-0.5 sm:gap-1 shrink-0 cursor-pointer transition-all"
            >
              <Crown className="w-3.5 h-3.5 shrink-0 eb-warn" />
            ...
            <motion.button
              ...
              className="t-meta eb-warn px-1.5 sm:px-2 py-0.5 sm:py-1 bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/40 rounded-lg sm:rounded-xl transition-all flex items-center gap-0.5 sm:gap-1 cursor-pointer shrink-0 shadow-xs"
            >
              <Crown className="w-3.5 h-3.5 shrink-0 eb-warn" />
              <span>Get Pro</span>
            </motion.button>
          ...
          {isAdmin && (
            ...
            >
              <Shield className="w-3.5 h-3.5 shrink-0 sm:w-3.5 sm:h-3.5" />
              <span>Admin</span>
            </button>
          )}
          ...
          >
            {isMobileMenuOpen ? <X className="w-3.5 h-3.5 shrink-0 sm:w-4 sm:h-4" /> : <Sliders className="w-3.5 h-3.5 shrink-0 sm:w-4 sm:h-4 text-[var(--signal-ink)]" />}
          </button>
        </div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            {/* Language. Both options named, current one obvious — a single
                toggle left people guessing what it would switch to. */}
            <div className="mb-3">
              <p className="eb-label mb-2">Language</p>
              <div className="grid grid-cols-2 gap-2">
                {LANGUAGES.map((l) => {
                  const active = getLang() === l.id;
                  return (
                    <button
                      key={l.id}
                      onClick={() => {
            ...
            <div className="mt-3 pt-3 border-t border-[var(--rule)]/60 space-y-2.5">
              ...
              {currentUser ? (
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-[var(--signal)] text-white font-mono text-xs font-bold px-3.5 py-2 rounded-xl shadow-sm active:bg-[#4B5BE0] cursor-pointer"
                >
