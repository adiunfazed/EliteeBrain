...
import { X, Grid, ArrowLeft, Play, Check, XCircle, Sparkles, HelpCircle } from 'lucide-react';
...
export const SpatialNBackModule: React.FC<Props> = ({ currentLevel, onFinishSession, onClose }) => {
  ...
  /**
   * Build the flash sequence with roughly a third matches.
   *
   * Two guards matter here. At n=1 a "forced match" means repeating the
   * immediately previous square, so at 35% probability the same box could
   * light up five or six times in a row — which looks broken rather than
   * challenging. And a non-match must not accidentally repeat the previous
   * square either, or the player sees a match that the game does not score.
   */
  const generateSequence = useCallback(() => {
    const seq: number[] = [];
    // How many times the current square has repeated back-to-back.
    let runLength = 0;

    for (let i = 0; i < TOTAL_FLASHES; i++) {
      const target = i >= nValue ? seq[i - nValue] : -1;
      const previous = i > 0 ? seq[i - 1] : -1;

      // Allow a forced match only if it will not extend a visible run.
      const wouldRepeat = target === previous;
      const canForceMatch = i >= nValue && (!wouldRepeat || runLength < 1);

      if (canForceMatch && Math.random() < 0.35) {
        seq.push(target);
        runLength = target === previous ? runLength + 1 : 0;
        continue;
      }

      // Pick from squares that are neither the n-back target nor the previous
      // square, so a non-match is unambiguous and never looks like a repeat.
      const options: number[] = [];
      for (let sq = 0; sq < 9; sq++) {
        if (sq === target) continue;
        if (sq === previous) continue;
        options.push(sq);
      }
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-950 text-slate-100 overflow-y-auto">
      ...
      <div className="flex items-center gap-3 p-3 md:px-8 border-b border-slate-800/80 bg-slate-900/60 backdrop-blur-lg">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          <Grid className="w-5 h-5 shrink-0 eb-done" />
          <h2 className="text-[15px] font-bold text-slate-100 truncate min-w-0">Fluid IQ ({nValue}-Back)</h2>
          <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-emerald-500/10 eb-done border border-emerald-500/30 shrink-0 whitespace-nowrap">
            Lvl {currentLevel}
          </span>
        </div>
