      ...
      >
        {grid.map((filled, i) => {
          ...
          return (
            ...
            >
              {filled && isAnchor && showGuide && <Sparkles className="w-3.5 h-3.5 shrink-0 text-slate-950 fill-current" />}
            </div>
          );
        })}
      </div>
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-950 text-slate-100 overflow-y-auto">
      ...
      <div className="flex items-center gap-3 p-3 md:px-8 border-b border-slate-800/80 bg-slate-900/60 backdrop-blur-lg">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <h2 className="text-[15px] font-bold text-slate-100 truncate min-w-0">3D Visuospatial Rotation</h2>
          <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/30 shrink-0 whitespace-nowrap">
            Lvl {currentLevel}
          </span>
        </div>
        ...
        <div className="flex items-center gap-3">
          {sessionStartTime && (phase === 'active' || phase === 'feedback') && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 font-mono text-xs">
              <Timer className="w-3.5 h-3.5 shrink-0 text-indigo-400 animate-pulse" />
        ...
        {(phase === 'active' || phase === 'feedback') && (
          <div className="w-full text-center">
            ...
            <div className="grid grid-cols-2 gap-4 bg-slate-900 border-2 border-indigo-500/40 p-6 rounded-xl shadow-none mb-4 max-w-md mx-auto items-center">
              ...
              <div className="flex flex-col items-center">
                <span className="t-meta text-indigo-400 uppercase mb-2">Rotated ({rotationAngle}°)</span>
