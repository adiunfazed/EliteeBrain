      ...
      >
        {items.map((_, idx) => (
          <React.Fragment key={idx}>
            {tile.shape === 'circle' && (
              <div className="w-6 h-6 rounded-full border-2 shadow-sm" style={{ borderColor: tile.color, backgroundColor: `${tile.color}25` }} />
            )}
            {tile.shape === 'square' && (
              <div className="w-6 h-6 border-2 rounded-md shadow-sm" style={{ borderColor: tile.color, backgroundColor: `${tile.color}25` }} />
            )}
            {tile.shape === 'triangle' && (
              <div
            ...
            {tile.shape === 'diamond' && (
              <div className="w-5 h-5 border-2 rotate-45 shadow-sm" style={{ borderColor: tile.color, backgroundColor: `${tile.color}25` }} />
            )}
            {tile.shape === 'star' && <Sparkles className="w-6 h-6 drop-shadow-sm" style={{ color: tile.color }} />}
          </React.Fragment>
        ))}
      </div>
    );
