...
export const Dashboard: React.FC<Props> = ({
  ...
  useEffect(() => {
    if (activeSection === 'hub') setHubPane('today');
  }, [activeSection]);
        ...
        {activeSection === 'hub' && (
          ...
          >
            ...
            <div className="flex items-center gap-1.5">
              {(['today', 'tasks', 'goals', 'routine', 'focus'] as const).map((p) => (
                <button
                  key={p}
                ...
                >
                  {p === 'today' ? 'Today' : p === 'tasks' ? 'Tasks' : p === 'goals' ? 'Goals' : p === 'routine' ? 'Routine' : 'Focus'}
                </button>
              ))}
            </div>
