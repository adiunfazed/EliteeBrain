...
export const Dashboard: React.FC<Props> = ({
  ...
  const [hubPane, setHubPane] = useState<'tasks' | 'focus' | 'goals' | 'life' | 'community'>('tasks');
        ...
        {activeSection === 'hub' && (
          ...
          >
            ...
            <div className="flex items-center gap-1.5">
              {(['tasks', 'focus', 'goals', 'life', 'community'] as const).map((p) => (
                <button
                  key={p}
                ...
                >
                  {p === 'tasks' ? 'Tasks' : p === 'focus' ? 'Focus' : p === 'goals' ? 'Goals' : p === 'life' ? 'Life' : 'Community'}
                </button>
              ))}
            </div>
