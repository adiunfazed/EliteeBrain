    ...
    if (result.levelChange === 'up') {
      return (
        <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/40 eb-done px-4 py-2 rounded-xl">
          <TrendingUp className="w-5 h-5 shrink-0 eb-done animate-bounce" />
          <div>
    ...
    if (result.levelChange === 'down') {
      return (
        <div className="flex items-center gap-2 bg-rose-500/10 border border-rose-500/40 eb-danger px-4 py-2 rounded-xl">
          <TrendingDown className="w-5 h-5 shrink-0 eb-danger" />
          <div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg bg-[var(--surface-sunk)] border border-[var(--rule)] rounded-3xl p-6 md:p-8 shadow-2xl overflow-hidden text-[var(--ink)]">
        ...
        <button
          ...
          className="w-full py-3.5 rounded-2xl bg-[#8B5CF6] hover:bg-[#8B5CF6]/90 text-white font-black text-sm tracking-wide cursor-pointer transition-all shadow-md flex items-center justify-center gap-2 active:scale-95"
        >
