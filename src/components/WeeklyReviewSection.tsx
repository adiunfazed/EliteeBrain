    ...
    <div className="space-y-4">
      ...
      <div className="eb-panel p-4 sm:p-5">
        <span className="eb-label flex items-center gap-1.5">
          <Compass className="w-3.5 h-3.5 shrink-0 text-[var(--signal-ink)]" />
        ...
        <div className="mt-3 space-y-2.5">
          {areas.map((a) => (
            <div key={a.key} className="flex items-center gap-3">
              ...
              <span
                className={`t-meta tabular-nums w-10 text-right shrink-0 ${
      ...
      <div className="eb-panel p-4 sm:p-5">
        <span className="eb-label flex items-center gap-1.5">
          <CalendarCheck className="w-3.5 h-3.5 shrink-0 eb-done" />
          This week
        </span>
          ...
          <>
            ...
            {review.wentWell.length > 0 && (
              <div className="mt-3.5">
                <p className="t-meta eb-done tracking-widest uppercase">
                  What went well
                </p>
            ...
            {review.missed.length > 0 && (
              <div className="mt-3.5">
                <p className="t-meta eb-warn tracking-widest uppercase">
                  What slipped
                </p>
                <ul className="mt-1.5 space-y-1">
                  {review.missed.map((t) => (
                    <li key={t} className="text-[11px] text-[var(--ink-muted)] flex items-start gap-2">
                      <AlertCircle className="w-3.5 h-3.5 eb-warn shrink-0 mt-0.5" />
                      {t}
                    </li>
                  ))}
                </ul>
            ...
            {review.oneThing && (
              <div className="mt-3.5 p-3 rounded-xl bg-[color-mix(in_oklab,var(--signal)_10%,transparent)] border border-[var(--signal)]/25">
                <p className="t-meta text-[var(--signal-ink)] tracking-widest uppercase">
                  One thing next week
                </p>
                ...
                {onGo && review.bottleneck && (
                  <button
                    ...
                    className="eb-btn-primary mt-3 px-3.5 py-2 t-meta flex items-center gap-1.5"
                  >
                    Fix it now
