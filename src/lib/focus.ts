...
export interface ActiveFocus {
  taskId?: string;
  /** When set, completed minutes are added to this habit for today. */
  habitId?: string;
  taskTitle: string;
...
export function startFocus(
  ...
  return { taskId, habitId, taskTitle, plannedMinutes, startedAt: now, pausedMs: 0, pausedAt: null };
}
...
export async function saveFocusSession(
  ...
  if (!userId || !db) return;
  // Same undefined hazard as tasks: taskId is often absent. Sessions are flat,
  // but this stays consistent with the deep strip used elsewhere.
  const payload: Record<string, any> = {};
  for (const [k, v] of Object.entries(session)) if (v !== undefined) payload[k] = v;
  await setDoc(doc(db, 'users', userId, 'focusSessions', session.id), payload, { merge: true });
}
