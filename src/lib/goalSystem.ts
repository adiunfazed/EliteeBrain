import type { Goal, GoalHealth, Habit, HabitLog, RoutineBlock, RoutineLog, Task } from '../types';
import { todayISO } from './tasks';
import { habitStats } from './habits';
import { isBlockOn } from './routine';
...
export interface GoalProgress {
  ...
  source: 'value' | 'milestones' | 'habits' | 'tasks' | 'none';
}
...
export function goalProgress(
  ...
  // 2. Habit-driven goals: average consistency of everything scheduled toward
  //    the goal — linked habits AND linked routine blocks. A block you tick off
  //    is execution toward the goal exactly as much as a habit is.
  if (goal.metric === 'habit') {
    const linkedHabits = habits.filter((h) => h.goalId === goal.id && h.status !== 'archived');
    const linkedBlocks = blocks.filter((b) => b.goalId === goal.id && b.active);

    if (linkedHabits.length === 0 && linkedBlocks.length === 0) {
      return { ratio: 0, percent: 0, label: 'Nothing linked yet', source: 'none' };
    }

    const parts: number[] = linkedHabits.map(
      (h) => habitStats(h, logs, today).completionRate / 100
    );

    if (linkedBlocks.length > 0) {
      // Adherence on the linked blocks over the last 14 days.
      let scheduled = 0;
      let done = 0;
      for (let i = 0; i < 14; i++) {
        ...
        for (const b of linkedBlocks) {
          if (!isBlockOn(b, iso)) continue;
          scheduled++;
          if (routineLogs.some((l) => l.blockId === b.id && l.date === iso && l.state === 'done')) {
            done++;
          }
        }
      }
      if (scheduled > 0) parts.push(done / scheduled);
    }

    const avg = parts.reduce((a, b) => a + b, 0) / parts.length;
    const bits: string[] = [];
    if (linkedHabits.length) bits.push(`${linkedHabits.length} habit${linkedHabits.length === 1 ? '' : 's'}`);
    if (linkedBlocks.length) bits.push(`${linkedBlocks.length} routine block${linkedBlocks.length === 1 ? '' : 's'}`);

    return {
      ratio: avg,
      percent: Math.round(avg * 100),
      label: `${bits.join(' + ')} · ${Math.round(avg * 100)}% consistency`,
      source: 'habits',
    };
  }
  ...
  // For 'count' goals with linked tasks, progress is DERIVED by counting the
  // completed linked tasks. Storing a running total and incrementing on each
  // completion would double-count on any re-sync, un-complete, or replay of
  // the same event — deriving it cannot.
  let current = goal.currentValue || 0;
  if (goal.metric === 'count') {
    const linked = tasks.filter((t) => t.goalId === goal.id);
    if (linked.length > 0) {
      current = linked.filter((t) => t.completed).length;
    }
  }
  if (target <= 0) {
  ...
  return {
    ...
    percent: Math.round(ratio * 100),
    label:
      goal.metric === 'count' && linkedCount > 0
        ? `${current} of ${target} linked tasks done`
        : `${current}${unit} of ${target}${unit}`,
    source: goal.metric === 'count' && linkedCount > 0 ? 'tasks' : 'value',
  };
}
